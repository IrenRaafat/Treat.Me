<template>
    <div>
        <h1 class="text-center p-3" style="color: #2b3467;">All Blogs</h1>
        ...
        <table class="table text-center text-light border mt-2">
            ...
            <tr class="table-light" v-for="(blog, index) in handleSearch" :key="index">
                <template v-if="index>=min && index< max">
                <td>{{index +1}}</td>
                <td>{{blog.name}}</td>
                <td><img :src="blog.imgCard" alt="" width="100" height="100"></td>
                <td>{{blog.overviewCard}}</td>
                <td>
                    <!-- <router-link :to="`/UpdateBlog${blog.id}`"><button class="btn btn-sm">
                    ...
                    <button class="btn btn-sm" @click="deletePrd(blog.id)">
